 ...
 *****************************************************************************/
//
// $Id: SVGPathSegArcAbsImpl.java 780 2001-03-27 17:24:21Z honkkis $
//
...
public class SVGPathSegArcAbsImpl extends SVGPathSegImpl implements SVGPathSegArcAbs {
	...
	public SVGPathSegArcAbsImpl(float x, float y, float r1, float r2, float angle, boolean largeArcFlag,
			boolean sweepFlag) {
		this.x = x;
		this.y = y;
		this.r1 = r1;
	...
	public SVGPoint getPointAtLength(float distance, SVGPoint startPoint, SVGPoint lastControlPoint) {
		...
		if (Math.abs(distance - getTotalLength(startPoint, lastControlPoint)) < 0.001) {
			...
			// Need to convert from endpoint to centre parameterization to find
			// extent angle
			double cosAngle = Math.cos(angle);
			double sinAngle = Math.sin(angle);
